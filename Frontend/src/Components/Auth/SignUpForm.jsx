...
const InstituteCodeStatusIndicator = ({ status, verifiedInstitute }) => {
  if (status === "checking") {
    return (
      <p className="mt-1.5 flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400">
        <Loader2 size={14} className="animate-spin" /> Checking code...
      </p>
    );
  }
  if (status === "valid") {
    return (
      <p className="mt-1.5 flex items-center gap-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400">
        <CheckCircle2 size={14} /> Joining {verifiedInstitute?.name}
      </p>
    );
  }
  if (status === "invalid") {
    return (
      <p className="mt-1.5 flex items-center gap-1.5 text-xs font-semibold text-rose-500">
        <XCircle size={14} /> No institute found with this code.
      </p>
    );
  }
  return (
    <p className="mt-1.5 text-xs text-slate-400 dark:text-slate-500">
      Ask your institute administrator for this code.
    </p>
  );
};
...
const SignUpForm = ({
  ...
  const isInstituteVerified = instituteCodeStatus === "valid";
  ...
  return (
    ...
    >
      <div className="space-y-1.5 md:col-span-2">
        ...
        />
        <InstituteCodeStatusIndicator
          status={instituteCodeStatus}
          verifiedInstitute={verifiedInstitute}
        />
      </div>

      <fieldset
        disabled={!isInstituteVerified}
      ...
      >
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            required
            className={inputClass}
            placeholder="Your institution's official enrollment number"
          />
          <p className="mt-1.5 text-xs text-slate-400 dark:text-slate-500">
            Not the same as your roll number — this is checked by your admin
            during approval.
          </p>
        </div>
      ...
      <div className="md:col-span-2">
        <button
          type="submit"
          disabled={isSubmitting || !isInstituteVerified}
          className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-indigo-600 px-5 py-3 text-sm font-black tracking-wide text-white shadow-lg shadow-indigo-900/25 transition hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-400/80 disabled:cursor-not-allowed disabled:opacity-50"
        >
