...
export default function FolderStructureDiagram() {
  ...
  const SEM_YS = [112, 164, 213, 295];
  const SUB_YS = [112, 164, 213, 295];
  const DOTS_Y = 257;
  const MY_CY = 195;

  const MY_RX = CX_MY + 60;
  const FAN_X = CX_SEM + 45;
  const SUB_RX = CX_SUB + 52.5;
  const NOTE_LX = CX_NOTE - 76;

  const LABEL_CY = 458;
  const LABEL_H = 36;
  const LABEL_TOP = LABEL_CY - LABEL_H / 2;

  const ALWAYS_CY = 540;
  const ALWAYS_H = 46;
  const ALWAYS_TOP = ALWAYS_CY - ALWAYS_H / 2;
