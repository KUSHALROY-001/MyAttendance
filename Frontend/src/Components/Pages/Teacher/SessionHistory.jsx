...
const SessionHistory = () => {
  ...
  const presentCount = session.students.filter(s => s.status === "Present" || s.status === "Late").length;
  const absentCount = session.students.filter(s => s.status === "Absent").length;
  const leaveCount = session.students.filter(s => s.status === "Leave").length;
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center space-x-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-900 tracking-tight">Session Details</h1>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <div className="flex items-center space-x-3 mb-2">
                ...
                <div>
                  <h2 className="text-lg font-bold text-gray-900 leading-tight">{session.courseName}</h2>
                  <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">{session.courseCode}</p>
                </div>
              </div>
              <p className="text-sm font-medium text-gray-500 mt-2">
                {session.department} • Sem {session.semester} • Section {session.section}
              </p>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
            <h3 className="text-sm font-bold text-gray-900">Student Attendance List</h3>
          </div>
          ...
          <div className="divide-y divide-gray-50">
            {session.students.map((student) => {
              ...
              return (
                <div key={student.id} className="flex items-center justify-between p-4 hover:bg-gray-50/50 transition">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">{student.rollNumber}</p>
                    </div>
                  </div>
                  
                  <div className={`px-3 py-1 rounded-md text-[11px] font-bold border ${statusColor} ${statusBorder}`}>
                    {student.status}
                  </div>
                </div>
              );
